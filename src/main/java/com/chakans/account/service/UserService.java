package com.chakans.account.service;
...
public class UserService {
    ...
    public User createUser(UserDTO userDTO) {
        User user = new User();
        user.setLogin(RandomUtil.getRandomUUID(userRepository, "findOneByLogin"));
        ...
        if (userDTO.getAgreements() != null) {
            user.setAgreements(userDTO.getAgreements());
        }
    ...
    @Transactional(readOnly = true)
    public Optional<UserDTO> getUserWithAuthoritiesAndAgreementsByLogin(String login) {
        return userRepository.findOneWithAuthoritiesAndAgreementsByLogin(login).map(UserDTO::new);
    }

    @Transactional(readOnly = true)
    public Optional<UserDTO> getUserWithAuthoritiesAndAgreements(Long id) {
        return userRepository.findOneWithAuthoritiesAndAgreementsById(id).map(UserDTO::new);
    }

    @Transactional(readOnly = true)
    public Optional<UserDTO> getUserWithAuthoritiesAndAgreements() {
        return SecurityUtils.getCurrentUserLogin().flatMap(userRepository::findOneWithAuthoritiesAndAgreementsByLogin).map(UserDTO::new);
    }
